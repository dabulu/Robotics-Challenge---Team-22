...
class colour_search(object):
    ...
    def __init__(self):
        ...
        self.searching_for_pillar = True
    ...
    def turn(self, target, right):
        ...
        if "right":
            ...
            turn_speed = -0.3
        while turning:
            self.robot_controller.set_move_cmd(0.0, turn_speed)
            self.robot_controller.publish()
            self.rate.sleep()
            print(abs(ref_yaw - self.get_yaw_as_bearing(self.robot_odom.yaw)))
    ...
    def main(self):
        while not self.ctrl_c:
            ...
            #     self.robot_controller.publish()
            #     self.rate.sleep()
            #     if abs(self.start_yaw - self.robot_odom.yaw) >= 90:
            #         self.turn = False
            #         self.get_colour = True
                    
            # self.start_yaw = self.get_yaw_as_bearing(self.robot_odom.yaw)
            # #turn back to initial position
            # while self.turn_back:
            #     self.robot_controller.set_move_cmd(0.0, -0.3)
            #     self.robot_controller.publish()
            #     self.rate.sleep()
            #     # print(abs(self.robot_odom.yaw - self.start_yaw))
            #     if abs(self.get_yaw_as_bearing(self.robot_odom.yaw) - self.start_yaw) >= 90:
            #         self.robot_controller.stop()
            #         self.turn_back = False
            
            # while self.searching_for_pillar:
            #     if self.counter == 1:
            ...
            #         if abs(self.robot_odom.posy) < 1.5260:
            #             #self.turn()
